...
test('installs the pinned crate once and reuses the cached binary', async (t) => {
  ...
  try {
    ...
    assert.match(
      cargoCalls[0],
      /^install coverage-mcp --version =0\.8\.2 --locked --bin coverage-mcp --root /,
    );
    assert.equal(
    ...
    await fs.access(
      path.join(temporary.directory, 'runtime', '0.8.2', 'bin', 'coverage-mcp'),
    );
  } finally {
    await temporary.cleanup();
  }
...
test('fails clearly when neither an exact binary nor Cargo is available', async (t) => {
  ...
  try {
    await assert.rejects(
      ...
      /Coverage MCP 0\.8\.2 is not installed and Cargo is unavailable/,
    );
  } finally {
    await temporary.cleanup();
  }
