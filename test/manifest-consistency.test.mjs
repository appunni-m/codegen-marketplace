...
test('testing plugin declares and documents its Coverage MCP contract', async () => {
  ...
  assert.deepEqual(compatibility.coverageMcp, {
    ...
    bootstrap: {
      ...
      defaultInstallRoot: '~/.coverage-mcp/runtime/0.8.3',
      installLockFile: '<runtime-dir>/.install-0.8.3.lock',
      releasePrerequisite: 'coverage-mcp 0.8.3 published on crates.io',
    },
    localDevelopment: {
      command: 'cargo',
      args: [
