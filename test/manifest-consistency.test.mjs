...
test('testing plugin declares and documents its Coverage MCP contract', async () => {
  ...
  assert.deepEqual(compatibility.coverageMcp, {
    ...
    bootstrap: {
      ...
      defaultInstallRoot: '~/.coverage-mcp/runtime/0.8.1',
      installLockFile: '<runtime-dir>/.install-0.8.1.lock',
      releasePrerequisite: 'coverage-mcp 0.8.1 published on crates.io',
    },
    localDevelopment: {
      command: 'cargo',
      args: [
