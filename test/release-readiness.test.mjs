...
test('release metadata uses stable versions and the pinned Coverage MCP bootstrap', async () => {
  const expectedVersions = new Map([
    ['plugins/testing/.claude-plugin/plugin.json', '0.8.0'],
    ['plugins/testing/.codex-plugin/plugin.json', '0.8.0'],
  ...
  assert.deepEqual(connector.env, { COVERAGE_MCP_VERSION: '0.16.0' });
  ...
  assert.equal(compatibility.coverageMcp.schemaRevision, 18);
  assert.equal(compatibility.coverageMcp.toolCount, 2);
  ...
  assert.equal(compatibility.coverageMcp.bootstrap.version, '=0.16.0');
