...
`./bin/coverage-mcp-launcher`, which reuses an exact 0.8.2 binary from
`COVERAGE_MCP_BIN`, `PATH`, or `~/.coverage-mcp/runtime/0.8.2`. On a cache miss,
one launcher acquires `.install-0.8.2.lock`, installs the exact published crate
...
version until Coverage MCP 0.8.2 exists on crates.io.
...
~/.coverage-mcp/runtime/0.8.2/bin/coverage-mcp connect \
...
```bash
cargo install coverage-mcp --version '=0.8.2' --locked --force
```
